...
// fieldExtractor copies a key/value metadata map (labels or annotations) onto
// resource attributes, honouring an allow-list / deny-list and a key prefix.
type fieldExtractor struct {
	enabled bool
	prefix  string
	include map[string]struct{} // empty => allow all
...
// inject writes the allowed entries of m onto attrs under the configured prefix.
...
// ownerExtractor writes the involved object's controlling owner reference
// (kind/name/uid) onto resource attributes.
...
// controllerOwner returns the ownerReference with controller=true, or nil if
// the object has no controlling owner.
