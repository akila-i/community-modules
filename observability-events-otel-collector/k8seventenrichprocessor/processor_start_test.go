...
// testSettings builds the minimal processor.Settings the processor reads (only
// the logger), avoiding the componenttest/processortest helpers so the test
// stays on packages already in go.mod.
...
// TestStartSyncsCachesAndEnriches drives the real informer path against a fake
// API server: Start must warm the caches, after which ConsumeLogs enriches from
// them. This is the cluster-free counterpart to the in-cluster smoke test.
...
// TestStartSkippedWhenEnrichmentDisabled verifies that with every source off the
// processor is a pure pass-through: Start builds no client and warms no caches
// (so it needs no workload RBAC), and Shutdown is safe despite Start being a
// no-op.
...
// TestShutdownIsIdempotent guards the stopInformers contract: Start's error
// path and Shutdown both call it, so a second call must not panic on a
// double close.
