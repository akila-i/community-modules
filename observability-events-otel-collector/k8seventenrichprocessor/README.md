...
<!-- status autogen section -->
| Status        |               |
| ------------- | ------------- |
| Stability     | [alpha]: logs |
| Distributions | [openchoreo]  |
| Warnings      | [Single instance, Memory consumption](#caveats) |
| Code Owners   | OpenChoreo Observability team |
<!-- end autogen section -->

The `k8seventenrich` processor enriches Kubernetes **events** with metadata of
the object that triggered the event (the event's *involved object*) — its
...
(All prefixes are configurable.) Each source can be enabled and filtered
...
processors:
  k8seventenrich:
    ...
    labels:
      ...
      exclude: []   # deny-list; applied even when include is empty
    annotations:
      enabled: false   # opt-in: annotations are noisy and may be sensitive
...
| `labels`        | block    | enabled | Enrichment from the involved object's labels. See *Field options*. |
| `annotations`   | block    | **disabled** | Enrichment from the involved object's annotations (noisy and possibly sensitive — opt in). See *Field options*. |
| `owner_references` | block | enabled | Enrichment from the involved object's controlling owner reference. See *Owner options*. |
...
### Example 2: Only OpenChoreo labels, plus selected annotations
...
### Example 4: Full events pipeline
...
Watched kinds are declared in `kinds.go`. Adding one is a single map entry plus a
matching RBAC grant — for example, to also enrich `Endpoints` events:
...
> Only **namespaced** kinds are supported, because the lookup requires a
> namespace. Cluster-scoped kinds (Node, PersistentVolume, …) are skipped.
...
kind: ServiceAccount
metadata:
  name: otel-collector-openchoreo
...
- **Memory.** Caches hold *all* objects of *all* watched kinds cluster-wide.
...
- Optional UID-based de-duplication of in-session `count` updates (see the
  repository `ARCHITECTURE.md`); restart-replay dedup is already handled by the
  receiver's `storage` extension without code.
