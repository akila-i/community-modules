...
exporters:
  opensearch:
    ...
    logs_index_time_format: "yyyy-MM-dd"   # daily indices: k8s-events-YYYY-MM-DD
    http:
...
> Need a pipeline the structured values don't cover? Set `configOverride` to a
...
```bash
make build         # OCB build → ./dist/otelcol-openchoreo-events
